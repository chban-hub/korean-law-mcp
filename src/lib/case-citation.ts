...
// 사건부호에 오지 않는 음절 — 조문·날짜·수량 표기를 사건번호로 오인하지 않게 막는다
// ("제999조의9" → 999조의9, "총 20건 3개" → 20건3). 사건부호 후보에서 이 음절들을 뺀다.
//
// 부호 목록을 화이트리스트로 열거하지 않는 이유: 실제 사건부호는 법원별로 계속 늘고
// (compact-query-planner.ts:66의 CASE_CODE_PATTERN도 므·후·허·재를 빠뜨린다), 누락은
// 실존 인용을 조용히 무검증 통과시킨다 — 환각 억제 도구에서 가장 나쁜 실패다.
// 반대로 과추출은 '미확인' 한 줄을 더 낼 뿐이라, 비대칭이 블랙리스트를 정당화한다.
// 조·항·호는 `(?<![\d제])` 선행 가드가 이미 대부분 막으므로, 실존 부호와 겹치는
// 회(회생: 회합·회단)·호(가정보호)는 넣지 않는다.
const NON_CASE_SYLLABLES = "조항목의년월일건명개원번쪽시분"
